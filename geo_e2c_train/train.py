...
def train(args) -> dict:
    ...
    # ── map-pyramid SOURCE strategy (dispatched at the two build_V points) ─────────────
    ...
    if args.map_pyramid_source == "legacy_token_partition":
        ...
        for r in tqdm(sorted(set(pair_trow)), desc="preload grids", unit="tile"):
            tiles.grid(tid[r])                                      # warm the RAM cache (legacy only)
    else:
        print(f"[pairs] {len(pairs)} canonical train pairs | native source (S cache, no grid preload)",
              flush=True)
    ...
    # persist the resolved run config + native-cache fingerprint (mode is auditable off-checkpoint)
    ...
    for epoch in tqdm(range(args.epochs), desc=f"{args.agg}/{args.pyramid_mode} epochs", unit="ep"):
        ...
        if do_eval:
            ...
            galV = build_gallery_V(model, eval_tiles, sr["val"].tile_ids, args.eval_chunk,
                                   dev, progress=True, build_V_fn=_bvf)  # built ONCE; val+test share it
            for w in ("val", "test"):
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--map-pyramid-source", choices=["legacy_token_partition", "native_hierarchical"],
                    ...
                    help="map-cell feature source: legacy n×n token partition (default) or native "
                         "hierarchical COG crops (needs --native-cache from native_map_pyramid.build_cache)")
    ap.add_argument("--native-cache", default=None, help="native_hierarchical: path to the S cache H5")
    ap.add_argument("--native-backbone", default="dinov2_vitg14", help="native cache fingerprint check")
