...
250 m top (intermediate levels are irrelevant and are not generated):
    ...
    query_box   = centered_square(center=q,             side_m=250)   # TRUE metres
    pyramid_top = centered_square(center=pyramid.centre, side_m=250)   # TRUE metres
...
Units: 250/1000 are TRUE metres. Centres/points live in raw EPSG:3857 (Web-Mercator)
coordinates, inflated by 1/cos(lat), so each box is built with grid side
``true_m / cos(lat)`` (``centered_square_true``). At lat 48.5 (cos≈0.66) a nominal
250 m top spans ~377 grid units; without this scaling it would span only ~165 true m
and a stride-250 gallery's tops would leave gaps between tiles (few positives).
...
class PyramidTopIoU250Selector:
    ...
    def select(self, point: GeoPoint, gallery: GalleryIndex) -> Sequence[PositiveMatch]:
        ...
        reach = C.true_m_to_grid((PYRAMID_TOP_SIZE_M + QUERY_SIZE_M) / 2.0, point.lat) + self._tol
        ...
        for t in gallery.candidates_box(city, point.x, point.y, reach):
            ...
            # top derived concentric with the validated base centre (true-metre footprint)
            top_box = C.centered_square_true(t.center_x, t.center_y, t.lat, PYRAMID_TOP_SIZE_M)
